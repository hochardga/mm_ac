  ...
  return db.transaction(async (tx) => {
    ...
    if (existingSubmission) {
      ...
      return {
        ...
        nextStatus: existingSubmission.nextStatus as SubmitObjectiveResult["nextStatus"],
    ...
    if (!objective) {
      throw new Error(`Objective ${input.objectiveId} is not authored in manifest`);
    }
    ...
    const objectiveAttemptHistory = await tx.query.objectiveSubmissions.findMany({
      where: and(
